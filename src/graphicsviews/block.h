...
public:
    ...
    /**
     * Сортированный список блоков, лежащих на одном уровне с нашим центром.
     *
     * Поиск идёт влево+вправо или вниз+вверх в зависимости от @p orientation
     * последовательным рекурсивным перебором соответствующих контактов.
     *
     * Если на пути поиска среза попадаются мультиблоковые контакты, следующий
     * блок выбираются так, чтобы его (поперечная срезу) координата центра была
     * наиближайшей к соответствующей координате нашего центра.
     */
    QList<Block *> slice(Qt::Orientation orientation);

    /**
     * Сортированный список блоков, лежащих на одном уровне с нашим центром.
     *
     * Поиск идёт влево, вправо, вверх или вниз в зависимости от @p orientation
     * и от @p before последовательным рекурсивным перебором соответствующих
     * контактов.
     *
     * Если на пути поиска среза попадаются мультиблоковые контакты, следующий
     * блок выбираются так, чтобы его (поперечная срезу) координата центра была
