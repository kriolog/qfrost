...
def spline_avg_coeffs(x, y):
    """Коэффициенты сплайна зацикливаемой функции, заданной средними значениями.
    ...
    Дополнительные условия (делают сплайн "зацикливаемым"):
    1) Непрерывность при зацикливании - f_1(x_0) = f_N(x_N).
    2) Гладкость первого порядка при закиливании - f'_1(x_0) = f'_N(x_N).
...
def spline_avg_piecefunctions(x, y):
    """Куски сплайна зацикливаемой функции, заданной средними значениями.

    Параметры:
    x: N+1 значений аргумента, задающих N интервалов. Должен быть возрастающим!
    y: N средних значений функции для каждого из выделенных интервалов.

    Возвращается 2 массива функций: N квадратичных полиномов, задающих сплайн на
    на соответствующих интервалах, и столько же булевых функций для определения
    принадлежности к этим интервалам (для использования с `numpy.piecewise`).
    """
...
def interp_avg(x, y, num): 
    """Координаты сплайна зацикливаемой функции, заданной средними значениями.
