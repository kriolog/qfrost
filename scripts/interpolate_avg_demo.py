...
def main():
    ...
    if isinstance(x[0], dt.date):
        ...
        xaxis.set_minor_locator(mdates.MonthLocator(bymonthday=15))

        def month_roman(x, pos):
            return _roman(mdates.num2date(x).month)

        xaxis.set_major_formatter(mticker.NullFormatter())
        xaxis.set_minor_formatter(mticker.FuncFormatter(month_roman))

        for tic in xaxis.get_minor_ticks():
